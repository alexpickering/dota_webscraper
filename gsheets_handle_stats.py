...
def calc_all_heroes(datafile_name, request_dict,outfile_name):
    rows = []
...
def calc_requested_heroes(datafile_name, request_dict,outfile_name):
    ...
    with open(datafile_name, 'r') as file_obj:
        ...
        for hero, lvls in request_dict.items():
            ...
            for lvl in lvls:
                ...
                intelligence_cell_val = ''.join(['=',base_intelligence,'+',intelligence_growth,'*($B',new_index,'-1)'])
    ...
    print(outdf)
    outdf.to_csv('for_test3.csv',index=False)
